...
// ==========================================
// API验证地址
const EPHONE_VERIFY_API_URL = 'https://puppy-subscription-api.zeabur.app/api/verify';
...
async function ephoneVerify(account, password) {
    ...
    try {
        const res = await fetch(EPHONE_VERIFY_API_URL, {
            method: 'POST',
...
async function tryLogin() {
    ...
    if (result.success) {
        ...
        try {
            ...
            // 初始化数据库（如果 initDatabase 存在）
            if (typeof initDatabase === 'function') {
                initDatabase(account);
            }
            
            // 【关键修复】完全移除遮罩层，确保不阻挡页面
            const overlay = document.getElementById('login-overlay');
            if (overlay) {
                overlay.style.transition = 'opacity 0.5s ease';
                overlay.style.opacity = '0';
                overlay.style.pointerEvents = 'none'; // 立即禁用点击事件
                setTimeout(() => {
                    if (overlay && overlay.parentNode) {
                        overlay.parentNode.removeChild(overlay); // 完全移除DOM节点
                    }
                }, 500);
            }
...
function initEphoneLogin() {
    ...
    if (isAuthenticated === 'true' && savedAccount) {
        console.log(`[EPhone Login] 检测到已登录账号: ${savedAccount}`);
        try {
